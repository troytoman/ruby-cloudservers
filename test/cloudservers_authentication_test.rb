...
class CloudserversAuthenticationTest < Test::Unit::TestCase
  ...
  def test_good_authentication
    ...
    connection = stub(:authuser => 'myuser', :authkey => 'bad_key', :auth_host => "a.b.c", :auth_port => "443", :auth_scheme => "https", :auth_version => '/1.0', :authok= => true, :authtoken= => true, :svrmgmthost= => "", :svrmgmtpath= => "", :svrmgmtpath => "", :svrmgmtport= => "", :svrmgmtscheme= => "", :proxy_host => nil, :proxy_port => nil)
  ...
  def test_bad_authentication
    ...
    server.stubs(:get).returns(response)
    Net::HTTP.stubs(:new).returns(server)
    connection = stub(:authuser => 'bad_user', :authkey => 'bad_key', :auth_host => "a.b.c", :auth_port => "443", :auth_scheme => "https", :auth_version => '/1.0', :authok= => true, :authtoken= => true, :proxy_host => nil, :proxy_port => nil)
